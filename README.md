...
Helps with prop drilling. Context API allows you to create global variables to be used across the entire application
2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
actions are functions that manipulate the action object which then manipulates state.
reducers organize application state into one area that is then manipulated via the action object. It allows for more fluid communication/access
between components and app state. 
store is a replacement for useReducer. This is the thing that actually creates/holds the state of your app and facilitates the functioning of the reducer. It is known as the single source of truth because this is where the immutable app state is held. 
3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
redux thunk allows us to create functions that can perform multiple different actions in the same function. It is primarily used for organization and API calls. 
4. What is your favorite state management system you've learned and this sprint? Please explain why!
Redux is definitely the most useful of them all, but in terms of simplicity, useReducer is nice. 
